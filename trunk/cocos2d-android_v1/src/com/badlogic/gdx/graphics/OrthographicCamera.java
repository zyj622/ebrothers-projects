...
public class OrthographicCamera extends Camera {
	 ...
	 * other unit scales (e.g. meters for box2d) proceed accordingly.
	 * 
	 * @param viewportWidth
	 *            the viewport width
	...
	/**
	 * Constructs a new OrthographicCamera, using the given viewport width and
	 * height. This will create a camera useable for iso-metric views. The
	 * diamond angle is specifies the angle of a tile viewed isometrically.
	 * 
	 * @param viewportWidth
	 *            the viewport width
	 * @param viewportHeight
	...
	public void findDirectionForIsoView(float targetAngle, float epsilon,
			int maxIterations) {
		...
		int iterations = 0;
		float aMid = 0;
		while (Math.abs(targetAngle - aMid) > epsilon
	...
	private Vector3 calculateDirection(float angle) {
		...
		// float rotAngle =
		// (float)Math.toDegrees(Math.asin(Math.tan(Math.toRadians(angle))));
	 ...
	 * the screen resolution, centered at (Gdx.graphics.getWidth()/2,
	...
	public void setToOrtho(boolean yDown) {
		setToOrtho(yDown, CCDirector.sharedDirector().winSize().width,
