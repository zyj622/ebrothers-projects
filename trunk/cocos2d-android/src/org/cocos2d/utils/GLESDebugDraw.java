...
public class GLESDebugDraw {
	...
	void drawCircle(CGPoint center, float radius, ccColor4F color) {
		...
		mGL.glColor4f(color.r, color.g, color.b, 1);
		CCDrawingPrimitives.ccDrawCircle(mGL, center, radius * mRatio, 0, 16,
				true);
	}
	...
	private void drawTransform(Transform xf) {
		...
		p2 = CGPoint.ccpAdd(p1, CGPoint.ccp(k_axisScale
				* xf.vals[Transform.COL1_X], k_axisScale
				* xf.vals[Transform.COL1_Y]));
		drawSegment(p1, p2, new ccColor4F(1, 0, 0, 1));

		p2 = CGPoint.ccpAdd(p1, CGPoint.ccp(k_axisScale
				* xf.vals[Transform.COL2_X], k_axisScale
				* xf.vals[Transform.COL2_Y]));
	...
	void drawAABB(World world, ccColor4F c) {
	...
	public void drawDebugData(GL10 gl) {
		...
		// draw AABB bit
		// if ((flag & DRAW_AABB) != 0) {
		// color = new ccColor4F(0.9f, 0.3f, 0.9f, 1);
	...
	private void drawShape(Fixture fixture, Transform xf, ccColor4F color) {
		if (Type.Circle == fixture.getType()) {
			...
			// CGPoint axis = CGPoint.ccp(xf.vals[Transform.COL1_X],
			// xf.vals[Transform.COL1_Y]);
			drawCircle(CGPoint.ccp(center.x, center.y), radius, color);
